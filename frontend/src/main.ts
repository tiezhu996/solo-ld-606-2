...
@Component({
  selector: "app-root",
  standalone: true,
  imports: [CommonModule, BerthsPage],
  template: `
  <div class="shell">
    ...
    <section class="workbench" *ngIf="active !== '/berths'"><div class="panel wide"><h2>业务数据</h2><article class="row" *ngFor="let item of entries"><strong>{{ item[0] }}</strong><span>{{ item[1].length }} 条</span><span class="badge">READY</span></article></div><div class="panel"><h2>联动检查</h2><p>Angular 路由、服务、构造器、常量和展示组件按提示词拆分。</p></div></section><app-berths-page *ngIf="active === '/berths'"></app-berths-page></main>
  </div>`
})
class AppComponent {
  routes = routes;
